...
if (isset($_POST['editCustomer'])) {
	...
	if (!empty($_POST['customer_id'])) {
		$customerId = mysqli_real_escape_string($connection, trim($_POST['customer_id']));
		$custIdQuery = "customer_id = '{$customerId}'";
		array_push($editParam, $custIdQuery);
	}
	...
	if (!empty($editParam)) { // If there's something to edit
		...
		$updateSql = "UPDATE customers SET " .$addendum . " WHERE customer_id = '{$id}' LIMIT 1";
...
$smarty->display('resources/smarty/templates/edit_customer.tpl');
?>